...
impl BierStlv {
    pub(crate) fn decode(_tlv_len: u16, buf: &mut Bytes) -> DecodeResult<Self> {
        let buf_len = buf.len();
        let tlv_len_err = DecodeError::InvalidTlvLength(buf_len as u16);

        let sub_domain_id =
            buf.try_get_u8().map_err(|_| tlv_len_err.clone())?;
        let mt_id = buf.try_get_u8().map_err(|_| tlv_len_err.clone())?;
        let bfr_id = buf.try_get_u16().map_err(|_| tlv_len_err.clone())?;
        let bar = buf.try_get_u8().map_err(|_| tlv_len_err.clone())?;
        let ipa = buf.try_get_u8().map_err(|_| tlv_len_err.clone())?;
        let _reserved = buf.try_get_u16().map_err(|_| tlv_len_err.clone())?;
        ...
        while buf.remaining() >= TLV_HDR_SIZE as usize {
            // Parse Sub-TLV type.
            let stlv_type =
                buf.try_get_u16().map_err(|_| tlv_len_err.clone())?;

            // Parse and validate Sub-TLV length.
            let stlv_len =
                buf.try_get_u16().map_err(|_| tlv_len_err.clone())?;
            let stlv_len_err = DecodeError::InvalidTlvLength(stlv_len);

            let stlv_wlen = tlv_wire_len(stlv_len);
            if stlv_wlen as usize > buf.remaining() {
                return Err(stlv_len_err.clone());
            }
...
impl RouterInfoCapsTlv {
    pub(crate) fn decode(tlv_len: u16, buf: &mut Bytes) -> DecodeResult<Self> {
        ...
        let caps = buf
            .try_get_u32()
            .map_err(|_| DecodeError::InvalidTlvLength(buf.len() as u16))?;
            ...
            match stlv_type {
                SUBTLV_SID_LABEL => {
                    let sid = match stlv_len {
                        4 => Sid::Index(buf_stlv.try_get_u32().map_err(
                            |_| DecodeError::InvalidTlvLength(stlv_len),
                        )?),
                        3 => {
                            let label =
                                buf_stlv.try_get_u24().map_err(|_| {
                                    DecodeError::InvalidTlvLength(stlv_len)
                                })? & Label::VALUE_MASK;
